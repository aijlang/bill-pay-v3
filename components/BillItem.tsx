...
const BillItem: React.FC<BillItemProps> = ({ bill, onMarkAsPaid, onUndoPayment, onDeleteBill, onEditBill, balance = 0 }) => {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`bg-white rounded-xl shadow-md border overflow-hidden transition-all duration-300 flex flex-col ${paid ? 'border-green-400 bg-green-50' : 'border-slate-200'} ${isDragging ? 'shadow-lg' : ''}`}
        >
